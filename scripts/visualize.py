        ...
        for j in range(num_classes):
            if i < j:
                ...
                new_cmap = LinearSegmentedColormap.from_list(
                    'custom_cmap', new_colors, N=2)
                save_path_temp = os.path.join(save_path, f'class_{i}{j}.png')
                pca_results = PCA(n_components=2).fit_transform(X_temp)
                fig, ax = plt.subplots(
                    figsize=(7, 5), sharey=True, sharex=True, dpi=400)
                plt.scatter(
                    pca_results[:, 0], pca_results[:, 1], c=Y_temp, cmap=new_cmap, s=5)
                plt.title(f'Class {i} and Class {j}', fontsize=15,
...
def main():
    ...
    parser.add_argument(
        "--resume", default='exp_output\\Cora\\asymm_noise_ratio_0.3\ckpt\\best_model.pth')
    parser.add_argument("--corrupt_ratio",
                        dest="corrupt_ratio", default=0.3, type=float)
    ...
    save_path = os.path.join(
        'imgs', args.dataset, f'{args.corrupt_type}_noise_ratio_{args.corrupt_ratio:.1f}')
