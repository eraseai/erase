...
def train(args,model, data,Y_all,loader,optimizer,epoch):
    ...
    for batch_size, n_id, adjs in tqdm(loader, leave=False, desc=f"Epoch {epoch}", dynamic_ncols=True):
        ...
        optimizer.zero_grad()
        out = model(x, adjs) # out.shape = (n_id.shape[0], out_channels)
...
@torch.no_grad()
def test(args,model, data,clean_y,L, loader, split_idx,evaluator):
    model.eval()
    with torch.no_grad():
        out = model.inference(data.x, subgraph_loader=loader)
        out = out.detach()
        noisy_train_labels = L[split_idx['train']]
        train_acc,valid_acc,test_acc = Linear_classifier(args,out, split_idx,noisy_train_labels,clean_y, evaluator)
...
def get_objs(args):
    ...
    dataset = PygNodePropPredDataset(name=args.dataset, transform=transforms,root='data')
    res['dataset'] = dataset
    data = dataset[0]
    res['data'] = data
...
def main():
    ...
    parser.add_argument('--n_embedding', type=int, default=512,
                        help='dimension of embedding dimension (default: )')
    ...
    parser.add_argument('--n_layers', type=int, default=2,help=
                        'number of layers of model (default: 2)')
    ...
    train_loader = NeighborSampler(data.edge_index, node_idx=train_idx, sizes=args.n_layers * [5],
    ...
    while run < args.runs:
        ...
        for epoch in range(args.epochs):
            ...
            if epoch % args.eval_steps == 0:
                Y_all = compute_semantic_structral_label(args,data,model,subgraph_loader,denoised_y[train_idx],train_idx)
                L = torch.argmax(Y_all,dim=1).reshape(-1,1)
                train_acc, valid_acc, test_acc = test(args,model, data,clean_y,L, subgraph_loader, split_idx,evaluator)
                logger.info(
                    f'Run: {run:02d},'
                    f'epoch: {epoch}'
