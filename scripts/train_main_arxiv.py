...
def train(args, model, data, Y_all, loader, optimizer, epoch):
    ...
    for batch_size, n_id, adjs in tqdm(loader, leave=False, desc=f"Epoch {epoch}", dynamic_ncols=True):
        ...
        criterion = MaximalCodingRateReduction(
            gam1=args.gam1, gam2=args.gam2, eps=args.eps)
...
def test(model, data, clean_y, L, loader, split_idx, evaluator):
    ...
    with torch.no_grad():
        ...
        train_acc, valid_acc, test_acc = Linear_classifier(
            out, split_idx, noisy_train_labels, clean_y, evaluator)
...
def main():
    ...
    parser.add_argument('--n_hidden', type=int, default=64,
                        help='dimension of hidden dimension (default: 256)')
    parser.add_argument('--n_embedding', type=int, default=512,
                        help='dimension of embedding dimension (default: 512)')
    parser.add_argument('--n_classes', type=int, default=40,
                        help='number of classes (default: 40)')
    parser.add_argument('--n_heads', type=int, default=8,
                        help='number of heads for attention module (default: 8)')
    parser.add_argument('--n_layers', type=int, default=2,
                        help='number of layers of model (default: 2)')
    ...
    parser.add_argument('--dataset', type=str, default='ogbn-arxiv',
                        help='dataset for training (default: ogbn-arxiv)')
    ...
    parser.add_argument('--eps', type=float, default=0.05,
                        help='eps squared (default: 0.05)')
    parser.add_argument('--alpha', default=0.6, type=float)
    parser.add_argument('--beta', default=0.5, type=float)
    parser.add_argument('--T', default=50, type=int)
    # corrupt settings
    parser.add_argument('--corrupt_type', default='asymm',
                        choices=['asymm', 'symm'])
    parser.add_argument("--corrupt_ratio",
                        dest="corrupt_ratio", default=0.5, type=float)
    # training settings
    parser.add_argument('--exp_dir', type=str, default='exp_output',
                        help='base directory for saving information. (default: '' )')
    ...
    args.exp_dir = os.path.join(
        args.exp_dir, args.dataset, f'{args.corrupt_type}_noise_ratio_{args.corrupt_ratio:.1f}')
    ...
    best_acc_lst = []
    # set up data loader
    train_loader = NeighborSampler(data.edge_index, node_idx=train_idx, sizes=args.n_layers * [
                                   5], batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, drop_last=True)
    subgraph_loader = NeighborSampler(data.edge_index, node_idx=None, sizes=[
                                      -1], batch_size=4096, shuffle=False, num_workers=args.num_workers)
    data = data.to(device)
    train_idx = split_idx['train']
    # corrupt the labels
    clean_y = data.y
    noisy_y = clean_y.clone()
    noisy_y[train_idx] = default_corrupt(
    ...
    while run < args.runs:
        ...
        for epoch in range(args.epochs):
            ...
            Y_all = compute_semantic_structral_label(
                args, data, model, subgraph_loader, denoised_y[train_idx], train_idx)
            # train the model
            loss = train(args, model, data, Y_all,
            ...
            if epoch % args.eval_steps == 0:
                ...
                train_acc, valid_acc, test_acc = test(
                    model, data, clean_y, L, subgraph_loader, split_idx, evaluator)
                logger.info(
                    f'Run: {run:02d},'
                    f'epoch: {epoch}'
                ...
                if valid_acc >= best_val_acc:
                    ...
                    torch.save(model.state_dict(), os.path.join(
                        args.exp_dir, 'ckpt', f'best_model.pt'))
