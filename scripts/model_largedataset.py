...
class GAT(torch.nn.Module):
    def __init__(self, base_layer, in_channels, hidden_channels, out_channels, num_layers, num_heads,
        ...
        self.layers.append(base_layer(
            in_channels, hidden_channels // num_heads, num_heads, **kwargs))
        ...
        self.layers.append(base_layer(
            hidden_channels, out_channels, 1, **kwargs))
        ...
        print(
            f"learnable_params: {sum(p.numel() for p in list(self.parameters()) if p.requires_grad)}")
        ...
        for i, (edge_index, _, size) in enumerate(adjs):
            ...
            if self.use_adj_norm:
                edge_index, edge_weight = self.norm(
                    edge_index, x.shape[0], edge_weight, improved=False, dtype=x.dtype)
    ...
    def inference(self, x, subgraph_loader, edge_weight=None):
        ...
        """
        pbar = tqdm(total=x.size(0) * len(self.layers),
                    leave=False, desc="Layer", disable=False)
            ...
            for batch_size, n_id, adj in subgraph_loader:
                ...
                if self.use_adj_norm:
                    edge_index, edge_weight = self.norm(
                        edge_index, x.shape[0], edge_weight, improved=False, dtype=x.dtype)
                    new_x = self.layers[i](
                        (x_source, x_target), edge_index, edge_attr=edge_weight)
                else:
