...
def Linear_classifier(args, data, features, noisy_train_labels, clean_labels):
    ...
    train_acc = accuracy(clf, train_features,
                         clean_labels[data.train_mask.cpu().numpy()])
    val_acc = accuracy(clf, val_features,
                       clean_labels[data.val_mask.cpu().numpy()])
    test_acc = accuracy(clf, test_features,
                        clean_labels[data.test_mask.cpu().numpy()])
