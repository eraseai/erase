...
def train(args,model,A,L_all,Y_all, data,optimizer,epoch):
    ...
    loss_func = MaximalCodingRateReduction(gam1=args.gam1, gam2=args.gam2, eps=args.eps)
    loss,loss_empi,loss_theo,L_all = loss_func(out,data,A,Y_all,args.alpha,args.beta,args.T,data.train_mask)
...
def test(args,model, data,noisy_y,clean_y):
    ...
    train_acc,valid_acc,test_acc = Linear_classifier(args,data,out, noisy_train_labels,clean_y)
...
def main():
    ...
    parser.add_argument("--dataset", dest="dataset", default='Cora', help='Cora, CiteSeer, PubMed',type=str)
    #ERASE settings
    #hyperparameter settings for reference: 
    #                gam1   gam2   eps   beta   alpha   T
    #Cora            1      2      .05   .6     .6      5
    #CiteSeer        1      2      .4    .7     .6      4
    #PubMed          1      3      .05   .6     .6      3
    parser.add_argument("--loss_func", dest="loss_func", default='ERASE', type=str)
    parser.add_argument("--gam1", dest="gam1", default=1, type=float)
    parser.add_argument("--gam2", dest="gam2", default=2, type=float)
    parser.add_argument("--eps", dest="eps", default=0.05, type=float)
    ...
    #other settings
    parser.add_argument("--exp_dir",dest='exp_dir',default='exp_output',type=str)
    ...
    for run in range(args.runs):
        ...
        best_val_acc = 0
        best_epoch = 0
        optimizer = torch.optim.Adam(model.parameters(),lr=args.lr, weight_decay=args.wd,amsgrad=True)
        for epoch in range(1, args.num_epoch+1):
            #train the model
            loss,loss_empi,L_all = train(args,model, A,L_all,Y_all,data,optimizer,epoch)
            ...
            if val_acc>best_val_acc:
                ...
                best_epoch = epoch
                torch.save(model.state_dict(), os.path.join(args.exp_dir, 'ckpt', f'best_model.pth'))
            logger.info(f'Run: {run:02d},'
                    f'Epoch: {epoch:02d}, '
